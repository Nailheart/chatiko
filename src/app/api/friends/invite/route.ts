...
const POST = async (req: Request) => {
  try {
    ...
    const isAlreadyRequested = await redis.sismember(
      `user:${userId}:incoming_friend_requests`,
      session.user.id,
    );
    if (isAlreadyRequested) {
      return NextResponse.json("You already requested this user.", {
        status: 400,
      });
    }
    ...
    if (isAlreadyAdded) {
      return NextResponse.json("This user is already on your friends list.", {
        status: 400,
      });
    }
